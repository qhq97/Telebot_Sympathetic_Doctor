...
class PrologReplier(telepot.aio.helper.ChatHandler):
    def __init__(self, *args, **kwargs):
        ...
        self.prolog = Prolog()
        self.prolog.consult('Talkingbox_2.pl')
    ...
    async def on_chat_message(self, msg):
        ...
        if (self.current_state == 'mood'):
            if (msg['text'] == '/start'):
                ...
                self.mood = mood[0]['X']
                question = 'Hi, I am Doctor Caring. How are you feeling today? Feeling ' + self.mood + '?'
                self.prolog.assertz('asked({})'.format(self.mood))
                kb = ReplyKeyboardMarkup(keyboard=[['Yes', 'No']])
                await bot.sendMessage(id, question, reply_markup=kb)
                ...
                kb = ReplyKeyboardMarkup(keyboard=[['Yes', 'No']])
                await bot.sendMessage(id, "Ok... Is it correct to say that you are not in pain?", reply_markup=kb)
